...
function PortableTextInputField({
  ...
  useEffect(() => {
    const currentHash = JSON.stringify(contentData);
    ...
    // If content changed and we didn't cause it, it's likely external
    if (lastContentRef.current !== null && lastContentRef.current !== currentHash) {
      setEditorKey((k) => k + 1);
    }
        ...
        >
          <div className="bg-zinc-900 rounded-xl border border-zinc-700 p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 id="image-modal-title" className="text-lg font-semibold text-white">Insert Image</h3>
              <Button
                type="button"
                variant="ghost"
                size="sm"
            ...
            {!pendingImageAssetId ? (
              <button
                ...
                disabled={isUploading}
                className={`w-full border-2 border-dashed border-zinc-600 rounded-lg p-8 text-center cursor-pointer hover:border-zinc-500 transition-colors bg-transparent ${isUploading ? "opacity-50 pointer-events-none" : ""
                  }`}
              >
                {isUploading ? (
                  <>
